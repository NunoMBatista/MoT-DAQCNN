...
quantum features, and support it with two ablations.
First, linear probing identifies the pairwise ZZ correlators as the
load-bearing ingredient of the feature map, adding $+0.025$ to $+0.040$ AUC
over single-qubit $\langle Z\rangle$ marginals depending on scale, whereas
digital and analog encoding produce features of comparable linear separability
on this task.
...
once an expressive trained head is added.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/capacity_sweep.pdf}
\caption{Test AUC on BreastMNIST as a function of head capacity for
four feature sources at two matched dimensionalities.
Top: single-kernel scale, quantum and classical-random sources matched
at $3{,}645$ features.
Bottom: four-kernel scale, matched at $14{,}580$ features.
Raw pixels (no feature extraction, $784$ inputs) are included as an
absolute floor.
Heads are ordered by architectural expressivity from Linear to
CNN-$64$.
Each cell is a $10$-seed mean with shaded $\pm 1$ standard-deviation
bands.}
\label{fig:capacity_sweep}
\end{figure}

\paragraph{Single-kernel scale (Fig.~\ref{fig:capacity_sweep}, top).}
At matched dimensionality of $3{,}645$ features, Digital-ZZ and
Analog-ZZ sit clearly above Random$\times 45$ at low-capacity heads:
the Linear-head AUCs are $0.845$ and $0.843$ versus $0.820$, and the
MLP-$1$ AUCs are $0.858$ and $0.844$ versus $0.828$.
The three feature substrates converge as head capacity grows, and the
ranking flips at the CNN-$64$ head:
Random$\times 45$ at $0.906 \pm 0.005$,
Analog-ZZ 1k at $0.900 \pm 0.008$,
Digital-ZZ 1k at $0.897 \pm 0.010$, with all differences inside one
standard deviation.
Raw pixels remain the lowest curve at every head, roughly $0.03$ AUC
below the quantum feature sources across the sweep and up to $0.04$
below Random$\times 45$ at CNN-$64$.

\paragraph{Four-kernel scale (Fig.~\ref{fig:capacity_sweep}, bottom).}
At matched dimensionality of $14{,}580$ features, the three
feature-based curves bunch tightly across the entire head sweep and no
source consistently leads any other beyond one standard deviation.
The maximum quantum-versus-random AUC gap at any head is $+0.015$
(Analog-ZZ vs Random$\times 180$, MLP-$2$); the minimum is
approximately $-0.002$ (Digital-ZZ vs Random$\times 180$).
We attribute this collapse to two factors: at $14{,}580$ dimensions a
...
As the classifier grows in capacity, the four curves in
Fig.~\ref{fig:capacity_sweep} converge: the trained CNN head extracts
additional class-relevant signal from the classical random substrate
almost as effectively as from the quantum substrate, closing the gap
and, at the single-kernel scale, reversing it by a margin smaller
...
unstructured random projections (mean AUC $0.772$, versus $0.766$ for
the higher-dimensional Random$\times 45$ baseline).
