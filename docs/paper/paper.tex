...
$\lfloor(28-k)/s\rfloor^2$ patches, each containing $k^2$ pixel values.
...
Schuld, Sweke and Meyer~\cite{schuld2021encoding} showed that any
data-encoding quantum circuit can be written as a partial Fourier series in the
input, with the accessible frequencies determined by the eigenvalue structure of
the encoding generator.
For digital encoding, the generator per qubit is $Y/2$, which has eigenvalues
$\pm\tfrac{1}{2}$, giving each pixel access to a single pair of frequencies.
For analog encoding, each pixel $x_i$ enters as the coefficient of $n_i$ in the
full many-body Rydberg Hamiltonian; the encoding generator is the entire
Hamiltonian, whose eigenvalue differences span a set of frequencies derived from
the $2^{k^2}$-dimensional many-body energy spectrum.
Under this framework, analog encoding provides a strictly richer function class
than digital encoding in the sense of accessible Fourier frequencies.

We emphasise that a wider frequency spectrum is a necessary, not sufficient,
condition for improved classification: whether the additional nonlinearities
align with the decision boundary of a given medical dataset is an empirical
question that our experiments are designed to answer.
...
In an entangled quantum state, the two-body observables
$\langle Z_i Z_j\rangle$ carry information about inter-qubit correlations that
is absent from the single-qubit marginals
($\langle Z_i Z_j\rangle \neq \langle Z_i\rangle\langle Z_j\rangle$ whenever
the state is entangled), extending the feature space from $k^2$ to
...
Table~\ref{tab:probing} reports linear SVM AUC on frozen feature maps for
BreastMNIST.
Several findings are consistent with our theoretical expectations.

\begin{table}[t]
\centering
\caption{Linear probing AUC on frozen features (BreastMNIST).
LinearSVC ($C\!=\!1$), features standardised on training split.
Primary comparison: Digital-ZZ vs.\ Analog-ZZ (matched dimensionality, same
topology).}
\label{tab:probing}
\begin{tabular}{lrr}
\toprule
Feature source & Dim. & AUC \\
\midrule
Raw pixels                    &    784 & 0.789 \\
\midrule
Random $\times$45 filters     &  3{,}645 & 0.766 \\
Digital-Z, 1-kern             &    729 & 0.775 \\
Digital-ZZ, 1-kern (kings)    &  3{,}645 & 0.793 \\
Analog-ZZ, 1-kern (kings)     &  3{,}645 & \textbf{0.810} \\
\midrule
Random $\times$180 filters    & 14{,}580 & 0.769 \\
Digital-ZZ, 4-kern            & 14{,}580 & \textbf{0.830} \\
Analog-ZZ, 4-kern             & 14{,}580 & 0.819 \\
\bottomrule
\end{tabular}
\end{table}
...
confirming that unstructured random projections of $3\!\times\!3$ patches are
not useful at this scale; the quantum interaction structure is a necessary
condition for the observed gain.
...
This is consistent with the Fourier expressibility argument: analog encoding
drives the quantum dynamics through a richer frequency spectrum than a single
$R_Y$ gate per qubit~\cite{schuld2021encoding}, producing feature vectors that
are more discriminative under a linear classifier.
...
We attribute this to the diversity of interaction graphs: four structurally
distinct topologies collectively span a richer feature space even under digital
encoding, partially compensating for its narrower per-kernel frequency spectrum.
This effect should be interpreted cautiously as the topology sets differ between
the single- and four-kernel configurations.
