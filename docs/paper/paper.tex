...
We support it with two compact linear-probing ablations.
...
The effect is dataset dependent: on the near-separable PneumoniaMNIST the
quantum sources are the \emph{weakest} at the linear probe, below raw pixels
and random projections, as the entangling transform disrupts already-accessible
structure; only on the lower-separability BreastMNIST do they lead at the
probe.
...
trained CNN; on the near-separable PneumoniaMNIST the quantum sources are in
...
Since a classical randomized nonlinear map of equal dimensionality (random
Fourier features) erases the only advantage we observe, the value of a Rydberg
kernel cannot rest on nonlinearity alone, which cheap classical methods already
provide.
...
that family.
Neither holds at the nine-qubit scale tested here: the features are precomputed
on a CPU and match cheap classical nonlinearity.
...
This is a prerequisite for trainable Rydberg kernels, which we regard as
promising but contingent: they must contend with the exponential-concentration
barrier, and where classical nonlinear maps already saturate performance,
optimizing the kernel may be a costlier route to the same ceiling.
