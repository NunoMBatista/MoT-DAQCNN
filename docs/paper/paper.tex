...
quantum-versus-classical-random AUC gap shrinks monotonically from
approximately $+0.025$ at a linear probe to about $-0.008$ at a trained CNN on
both datasets, indicating that the DAQCNN quantum advantage in this regime is
conditional on classifier capacity.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/capacity_sweep_pneumonia.pdf}
\caption{The same head-capacity sweep on PneumoniaMNIST, with identical
axes and feature sources to Fig.~\ref{fig:capacity_sweep}.
The capacity-conditioning pattern reproduces: quantum sources lead at
low-capacity heads and the ranking inverts under the trained CNN, where
Random$\times 45$ overtakes both ZZ variants.}
\label{fig:capacity_pneumonia}
\end{figure}

\paragraph{Replication on PneumoniaMNIST
(Fig.~\ref{fig:capacity_pneumonia}).}
The capacity-conditioning pattern reproduces on PneumoniaMNIST.
At single-kernel scale the averaged quantum-ZZ advantage over
Random$\times 45$ runs $+0.015$ at the Linear head, $+0.018$ at MLP-$1$,
$+0.010$ at MLP-$2$, $-0.001$ at CNN-$16$, and $-0.009$ at CNN-$64$:
positive at low capacity, eroding monotonically, and reversing under the
trained CNN, where Random$\times 45$ ($0.958$) overtakes both
Digital-ZZ and Analog-ZZ ($0.949$).
The ZZ-versus-Z gap is small throughout
($+0.009,\,+0.006,\,+0.010,\,-0.001,\,-0.004$ across the five heads),
consistent with the probe-level finding that correlators add little on
this near-separable task.
The four-kernel scale shows the same convergence.

\paragraph{Interpretation: two conditioning variables.}
The sweep isolates a single mechanism modulated by two factors.
The first is classifier capacity: on both datasets the
quantum-versus-random gap is largest at low capacity and shrinks
monotonically, reversing within one standard deviation under the trained
CNN head, as the head extracts class-relevant signal from the random
substrate almost as effectively as from the quantum one.
The second is task structure, visible in the ZZ ablation: the
correlators carry a large low-capacity benefit on BreastMNIST (whose
single-qubit features are only weakly separable, probe AUC $0.77$) and
almost none on PneumoniaMNIST (already separable from $\langle Z\rangle$
alone, probe AUC $0.92$).
A measurable quantum or ZZ advantage thus appears only when the head is
weak and the task is not already easy from simple features; relax either
condition and it vanishes.
Features matter most when the classifier matters least, and when the
task is hard enough to need them.
...
$+0.025$ AUC on BreastMNIST and $+0.015$ on PneumoniaMNIST, both at
single-kernel scale.
On both datasets this advantage shrinks monotonically as classifier
capacity grows and reverses sign at the highest capacity tested, where
