...
\subsubsection{Optimal Hyperparameters (1-Kernel Digital + ZZ)}
The hyperparameter search for the 1-Kernel Digital + ZZ configuration yielded a peak accuracy of $87.9\%$ using the \textbf{vertical} topology. The optimal classical classification head configuration utilized a batch size of $32$, a ReLU activation function, and a dropout rate of approximately $0.566$. For optimization, the search identified a learning rate of $0.00128$, a weight decay of $0.0001$, and a gradient clipping threshold of $7.83$. The model converged effectively utilizing an early stopping patience of $71$ epochs and a cosine annealing learning rate scheduler over a maximum of $100$ epochs. This specific tuning was critical in mitigating overfitting on the challenging grayscale BreastMNIST dataset while fully leveraging the expanded $ZZ$ feature space.
...
We investigate whether the classical classification head effectively utilizes the expanded feature space provided by the $ZZ$ measurements.
...
We evaluate the robustness of the ZZ-AQCNN framework across 10 independent validation seeds, as detailed in Figure \ref{fig:stability}.
...
\subsection{ZZ-AQCNN (Analog Quantum Convolutional Neural Networks)}
The inclusion of ZZ entanglement and the shift to analog Hamiltonian encoding represent a significant step towards more physically realistic and potentially more powerful Quantum CNNs. By including the $ZZ$ correlators, we increase the feature density per kernel from 9 to 45 (for a 3x3 kernel). Preliminary observations suggest that this richer feature space allows the classical classification head to draw more nuanced decision boundaries, particularly in the high-variance regions of the dataset.

A key finding of this work is the trade-off between the precision of digital gate-based encoding and the physical realism of analog Hamiltonian encoding. While digital encoding allows for exact state preparation, the analog mode more closely mirrors the natural evolution of neutral-atom processors. While the dynamic routing approach explored during the transition from QGARS proved challenging, the current results from the Analog ZZ search will define the new state-of-the-art for our architecture on medical images.
