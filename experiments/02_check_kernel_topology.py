...
def compute_edges(
    physical_coords: np.ndarray, scaling: float
...
def plot_topology(
    display_coords: np.ndarray, edges: list[tuple[int, int, float]], title: str
) -> None:
    fig, ax = plt.subplots(figsize=(5, 5))
    ax.scatter(
        display_coords[:, 0], display_coords[:, 1], c="tab:blue", s=120, zorder=3
    )
