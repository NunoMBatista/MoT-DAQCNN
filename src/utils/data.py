...
def get_pneumonia_mnist_loaders(cfg):
    ...
    try:
        ...
        raise ImportError(
            "medmnist is required for PneumoniaMNIST. Install with `pip install medmnist`."
        ) from exc
...
def get_breast_mnist_loaders(cfg):
    """Create train/val/test loaders for BreastMNIST via medmnist."""
    try:
        from medmnist import INFO, BreastMNIST
    except ImportError as exc:
        raise ImportError(
            "medmnist is required for BreastMNIST. Install with `pip install medmnist`."
        ) from exc
...
def get_dataloaders(cfg):
    ...
    if name == "breast_mnist":
        return get_breast_mnist_loaders(cfg)
