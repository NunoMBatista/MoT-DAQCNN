...
def get_rydberg_hamiltonian(wires, coordinates, scaling_factor=1.0):
    ...
    H_drive_base = qml.Hamiltonian([0.5] * n_qubits, ops_drive) # Combine the Sigma_X operators with the coefficients (without taking time into consideration) # type: ignore[arg-type]

    # Term B: Global Detuning (-Delta * Sum eta)
    # Coeff is -1.0 because formula is -Delta(t)
    ops_detuning = [projector_eta(w) for w in wires] # These are the eta operators in the detuning term (0 if ground, 1 if Rydberg)
    H_detuning_base = qml.Hamiltonian([-1.0] * n_qubits, ops_detuning) # Combine them in the Hamiltonian (without taking time into consideration) # type: ignore[arg-type]
    ...
    for i in range(n_qubits):
        for j in range(i + 1, n_qubits):
            ...
            delta = np.asarray(
                coordinates[i], dtype=float) - np.asarray(
